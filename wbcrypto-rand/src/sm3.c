...
#define wbcrypto_printf(...)  __android_log_print(ANDROID_LOG_DEBUG, "SECURITY_LIBRARY", __VA_ARGS__)
#else
...
void sm3_free(sm3_context *ctx) {
    if (ctx == NULL)
        return;
    wbcrypto_zeroize(ctx, sizeof(sm3_context));
}
